    ...
    *disconnected = false;
    ...
    try 
    {
        ...
        if (!messages.is_null())
        {
			...
			auto messagesArr = messages.as_array();
			if (!(messagesArr.cbegin() == messagesArr.cend()))
            {
				for (auto iter = messagesArr.cbegin(); iter != messagesArr.cend(); iter++)
                {
                    const value &v = *iter;
                    if (v.is_string())
                    {
                    ...
                    {
						connection->OnReceived(v.serialize());
                    }
                }
            }
